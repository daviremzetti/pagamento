...
public class MilitarConsultarPara extends javax.swing.JFrame {
    ...
    private void ListarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ListarActionPerformed
        ...
        if (matriculaDigitada.equals("   .   - ") && cpfDigitado.equals("   .   .   -  ") && nomeDigitado.isEmpty()) {
            ...
            filtrar(matriculaDigitada, nomeDigitado, cpfDigitado);
        }
        TfMatricula.setText("");
        TfCpf.setText("");
        TfNome.setText("");
    ...
    /**
     * Método para listar determinado militar por matrícula, nome ou cpf
     * @param matricula
     * @param nome
     * @param cpf 
     */
    private void filtrar(String matricula, String nome, String cpf) {
        DefaultTableModel tabelaModelo = (DefaultTableModel) TbLista.getModel();
        tabelaModelo.setNumRows(0);
        ...
        for (int i = 0; i < lista.size(); i++) {
            ...
            String id = String.valueOf(novoMilitar.getId());
            String matriculaMil = novoMilitar.getMatricula();
            String nomeMil = novoMilitar.getNome().toUpperCase();
            String posto = novoMilitar.getPostoGraduacao().getPosto_graduacao().toUpperCase();
            String lotacao = novoMilitar.getLotacao().getNome().toUpperCase();

            String[] linha = {
                id, matriculaMil, nomeMil, posto, lotacao
            };
            tabelaModelo.addRow(linha);
            TbLista.setModel(tabelaModelo);
        }
     ...
     */
    protected void selecionarParaContracheque() {
        try {
    ...
    /**
     * Método para selecionar militar para consulta de dependente
     */
    protected void selecionarParaConsulta() {
        try {
            int linhaSelecionada = TbLista.getSelectedRow();
            int idSelecionado = Integer.parseInt((String) TbLista.getValueAt(linhaSelecionada, 0));
    ...
    /**
     * Método para selecionar militar para cadastro de dependente
     */
    protected void selecionarParaCadastro() {
        try {
            int linhaSelecionada = TbLista.getSelectedRow();
            int idSelecionado = Integer.parseInt((String) TbLista.getValueAt(linhaSelecionada, 0));
    ...
    /**
     * Método para selecionar militar para cadastro de movimentação
     */
    protected void selecionarParaMovimentar() {
        try {
            int linhaSelecionada = TbLista.getSelectedRow();
            int idSelecionado = Integer.parseInt((String) TbLista.getValueAt(linhaSelecionada, 0));
    ...
    private void SelecionarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SelecionarActionPerformed
        switch (operacao) {
            case "consultarDependente" ->
                this.selecionarParaConsulta();
            case "movimentar" ->
                this.selecionarParaMovimentar();
            case "cadastrarDependente" ->
                this.selecionarParaCadastro();
            case "consultarContracheque" ->
                this.selecionarParaContracheque();
            default -> {
            }
        }

...
public class LotacaoConsultarPara extends javax.swing.JFrame {
    ...
    private void ConsultarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ConsultarActionPerformed
        ...
        if(TfBatalhao.getText().isEmpty() && TfCompanhia.getText().isEmpty() && TfPelotao.getText().isEmpty()) {
            ...
            filtrar(TfBatalhao.getText(), TfCompanhia.getText(),TfPelotao.getText());
        }
    ...
    /**
     * Método para listar todas as lotações em tela
     */
    private void listarLotacao() {

        DefaultTableModel tabelaModelo = (DefaultTableModel) TbLista.getModel();
        tabelaModelo.setNumRows(0);

        LotacaoDAO dao = new LotacaoDAO();
        List<Lotacao> lista = dao.listar();
    ...
    /**
     * Método para selecionar lotação por batalhão, companhia ou pelotão
     * @param batalhao
     * @param companhia
     * @param pelotao 
     */
    private void filtrar(String batalhao, String companhia, String pelotao) {

        DefaultTableModel tabelaModelo = (DefaultTableModel) TbLista.getModel();
        tabelaModelo.setNumRows(0);

        LotacaoDAO dao = new LotacaoDAO();
        List<Lotacao> lista = dao.filtrar(batalhao, companhia, pelotao);

        for (int i = 0; i < lista.size(); i++) {
            Lotacao novaLotacao = lista.get(i);
            String idLot = String.valueOf(novaLotacao.getId());
            String batalhaoLot = novaLotacao.getBatalhao();
            String companhiaLot = novaLotacao.getCompanhia();
            String pelotaoLot = novaLotacao.getPelotao();
            String nomeLot = novaLotacao.getNome().toUpperCase();

            String[] linha = {
                idLot, batalhaoLot, companhiaLot, pelotaoLot, nomeLot
            };
            tabelaModelo.addRow(linha);
            TbLista.setModel(tabelaModelo);
